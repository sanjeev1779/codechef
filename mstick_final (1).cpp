...
//float constructSTUtil_m(float arr[], int ss, int se, float *st, int si)
        int *st = constructST(b, N);
        //int *m_st=constructST_m(b,N);
        //int *m_st=constructST(b,N);
        scanf("%d",&Q);
        // Q=fastread();
        ...
        for(j=0; j<Q; j++) {
                ...
                minm= RMQ(st, N, L, R);
                float max_0_L=RMQ_m(st,N,0,L-1)+minm;
                float max_L_R=RMQ_m(st,N,L,R);
                float max_R_N=RMQ_m(st,N,R+1,N)+minm;
                float find_ma=float (minm+max_L_R)/2.0;
                max_0_L=maximum(max_0_L,max_R_N);
                float val=max_0_L*1.0;
                if(val>=find_ma){
                    printf("%.1f\n",val);
                }else {
                    printf("%.1f\n",find_ma);
...
/*
               ...
               // float *m_st=constructST_m(C,N);
              //  maxm= RMQ_m(m_st, N, 0, N-1);
                //printf("%.1f\n",maxm);
        }
