...
int maxVal(float x, float y)
{
        return (x > y)? x: y;
}

int RMQUtil_m(float *st, int ss, int se, int qs, int qe, int index)
{
        if (qs <= ss && qe >= se)
                return st[index];
...
float RMQ_m(float *st, int n, int qs, int qe)
{
        if (qs < 0 || qe > n-1 || qs > qe)
            {
                return -1;
